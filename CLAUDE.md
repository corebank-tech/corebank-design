...
**`src/lib`는 레이어 밖 레거시다.** 위 6개 레이어로 아직 해체되지 않은 상태이며, `eslint.config.js`의
`boundaries` 설정에도 임시 `legacy` 요소로 등록돼 있다. 신규 코드에서 `src/lib`를 새로 늘리지 않는다.
아래 "남은 마이그레이션" 참고.
...
| 조회 그리드 (A-94) | `src/widgets/query/` — data-grid, search-panel, search-fields, grid-toolbar, pagination, summary-row, empty-state |
...
| 메뉴 정의 | `src/lib/nav.ts` — 화면ID·경로 포함 (레거시 위치, 아래 "남은 마이그레이션" 참고) |
| HTTP·세션 | `src/shared/api/` — custom-fetch, query-client, api-error, session-events |
| Mock 서버 | `src/mocks/` — MSW 핸들러. `src/lib/mock/*`는 아직 화면 형태 목업으로 남아 있다 |
...
아래는 이번 스택 정비에서 규약만 확정하고 코드는 손대지 않은 항목이다. 다른 세션의 화면 작업과의
충돌을 피하기 위해 범위를 좁혔다 — 후속 커밋에서 순서대로 처리한다.

- `src/lib` 해체 → `entities/{account,transfer,product,auth,customer,notification,transaction}` +
  `shared/config/nav.ts`. `src/lib/mock/*` 21개는 MSW 핸들러(`src/mocks/handlers/`)로 전환하고,
  `openapi.yaml` 확정 후에는 Orval이 생성하는 핸들러로 대체한다
- 화면 파일 32개를 kebab-case + 소문자 화면ID 접두로 리네임 (예: `B01-AllAccounts.tsx` → `b01-all-accounts.tsx`)
- `interface` → `type` 전환 (122건), 컴포넌트 전용 Props export 해제 (69건 중 컴포넌트 외부에서
  실제로 소비하지 않는 ~63건)
- FSD 레이어 위반 7건 수정: `TermItem`·`ProductCard`·`ProductDetailData`(→ `entities/product`),
  `DataGrid`(`widgets/transfer/result-panel.tsx`가 `widgets/query`를 참조 → `shared/ui`로 하향),
  `NoticeBox`(`widgets/transfer/step-layout.tsx`가 `widgets/shell`을 참조 → `shared/ui`로 하향),
  `TermsAgreement`(`pages/auth`가 `pages/product`를 참조 → `widgets` 또는 `features`로 승격)
- 위 마이그레이션 완료 후: `.prettierignore`의 `src/pages`/`src/widgets`/`src/lib` 제외 제거,
  `eslint.config.js`의 `lint` 규칙을 `--max-warnings 0`으로 승격, `boundaries`의 `legacy` 요소와
  관련 예외 정책 삭제
