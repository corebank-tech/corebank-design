...
## 후속 조치 (2026-07-31) — xlsx 원본 동기화

`docs/requirements.md`가 가리키는 원본 `CoreBank_요구사항정의서_v1_4.xlsx`(구 v1_3)에도 위
POL-041·REQ-CMN-001·REQ-NFR-022·REQ-CMN-027 변경을 반영했다. 문서 자체의 개정 규칙(⑤ 요구사항
변경은 GitHub Issue → 기획 리드 승인 → 본 문서 갱신)에 따라 새 리비전 `v1.4`로 처리했다:

- `정책정의(POL)`: POL-041 기준값·설명 정정
- `기능요구사항`: REQ-CMN-001 상세설명·인수기준 개정, REQ-CMN-027(텍스트 크기 조절) 신설
- `비기능요구사항`: REQ-NFR-022(다크 테마) 신설
- `화면목록`·`추적성매트릭스`: A-90 행에 SideNav·REQ-CMN-027 반영
- `요구사항통계`: CMN 26→27, NFR 20→22로 갱신. NFR 항목은 v1.3에서 REQ-NFR-021 신설 시
  통계 시트가 갱신되지 않았던 기존 오류(21건인데 20건으로 표기)도 함께 정정했다
- `표지`·`개정이력`: 문서 버전 v1.4, 총 요구사항 212건(기능 190 / 비기능 22)으로 갱신,
  개정이력에 v1.4 행 추가
- 파일명을 `..._v1_3.xlsx` → `..._v1_4.xlsx`로 변경하고 `requirements.md`의 원본 참조를 갱신
