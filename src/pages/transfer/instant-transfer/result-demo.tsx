...
> = {
  ...
  fail: {
    ...
    footnote:
      "※ 실패한 이체는 원장에 반영되지 않으며, 잔액과 거래내역이 변동하지 않습니다.",
  },
  pending: {
    ...
    footnote:
      "※ 처리중 상태는 확정 전 임시 상태이며, 이체결과조회에서 확정 상태를 확인할 수 있습니다.",
  },
}
        ...
        <div className="flex items-center gap-1 rounded-[var(--radius-pill)] border border-[var(--color-border)] bg-surface p-1">
          {VARIANTS.map((v) => (
            <button
              ...
              className={
                "rounded-[var(--radius-pill)] px-3 py-1 text-sm font-bold whitespace-nowrap transition-colors " +
                (variant === v.id
